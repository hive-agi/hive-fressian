(ns build
  "Source jar + Clojars deploy, version read from ./VERSION (the same value
   release.yml tags as v{VERSION}) and coordinates from ./version.edn.
...
(defn jar
  "Build the source jar (pom + sources + resources) under target/."
  [_]
  (clean nil)
  (write-pom)
  (b/copy-dir {:src-dirs (conj src-dirs "resources") :target-dir class-dir})
...
(defn- already-published?
  "True if this exact lib+version jar is already on Clojars (which is
   immutable, so re-deploying an existing version errors)."
  []
...
(defn deploy
  "Build + deploy to Clojars. Requires CLOJARS_USERNAME and CLOJARS_PASSWORD
   (a deploy token) in the environment. No-ops if the version is already
   published (idempotent — safe to run on every main push)."
  [_]
